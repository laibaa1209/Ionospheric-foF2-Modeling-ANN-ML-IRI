"""
Linear Regression Model for foF2 Prediction
============================================
...
Outputs   : outputs/linear_model.joblib  (saved model)
            outputs/scatter_linear.png   (all-data scatter)
            outputs/predictions_linear.csv (all 37 035 rows)
"""
...
plt.title(f"Linear Regression — All Data\nR²={m_all['R2']:.4f}  RMSE={m_all['RMSE']:.4f}")
plt.legend(); plt.tight_layout()
plt.savefig(output_dir / "scatter_linear.png", dpi=150)
plt.close()
print(f"Scatter saved: outputs/scatter_linear.png")

# ── Save model ────────────────────────────────────────────────────────────────
joblib.dump(model, output_dir / "linear_model.joblib")
print(f"Model saved : outputs/linear_model.joblib")
...
}).to_csv(output_dir / "predictions_linear.csv", index=False)
print(f"Predictions : outputs/predictions_linear.csv  ({len(df):,} rows)")
