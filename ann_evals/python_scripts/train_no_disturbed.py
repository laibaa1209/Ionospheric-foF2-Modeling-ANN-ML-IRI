"""
Scenario 3 — Train on non-disturbed days, predict disturbed days.
=================================================================
Training data : master_no_disturbed.csv
Evaluation    : disturbed_days_only.csv

Saved to ann_evals/outputs/:
  ann_no_disturbed_model.pt
  ann_no_disturbed_loss_curve.png
  ann_no_disturbed_scatter.png
  predictions_ann_no_disturbed.csv

Usage:
    python train_no_disturbed.py
...
output_dir   = ann_evals / "outputs" / "scenario3_no_disturbed"
output_dir.mkdir(parents=True, exist_ok=True)

TRAIN_CSV = data_dir / "master_no_disturbed.csv"
EVAL_CSV  = data_dir / "disturbed_days_only.csv"
...
print("=" * 65)
print("SCENARIO 3 — Train on Non-Disturbed Days, Predict Disturbed Days")
print("=" * 65)
...
# ── Save checkpoint ───────────────────────────────────────────────────────────
ck_path = output_dir / "ann_no_disturbed_model.pt"
torch.save({
...
plt.title("Scenario 3 — No Disturbed Days: Training & Validation Loss")
plt.legend(); plt.grid(alpha=0.3); plt.tight_layout()
lc_path = output_dir / "ann_no_disturbed_loss_curve.png"
plt.savefig(lc_path, dpi=150); plt.close()
print(f"Loss curve: {lc_path}")

# ── Predict disturbed days ────────────────────────────────────────────────────
model.eval()
with torch.no_grad():
    y_pred = model(to_t(X_eval_n)).cpu().numpy().flatten()

print(f"\nDISTURBED DAYS  MSE={mean_squared_error(y_eval, y_pred):.4f}  "
...
scatter_plot(y_eval, y_pred, s_eval,
             "Scenario 3 — Disturbed Day Predictions",
             output_dir / "ann_no_disturbed_scatter.png")

# ── Save predictions ──────────────────────────────────────────────────────────
pred_path = output_dir / "predictions_ann_no_disturbed.csv"
pd.DataFrame({
    "Station":   s_eval,
